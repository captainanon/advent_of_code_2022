...
def solve_puzzle(file, rounds):
    ...
    for monkey in monkeys:
        mods.append(monkeys[monkey]['test'])
    mod = reduce(lambda x, y: x * y, mods)
    for round in range(rounds):
        for monkey in monkeys:
            ...
            for item in items:
                ...
                monkeys[pass_to]['items'].append(new_value % mod)
    inspection_counts = []
    for monkey in monkeys:
...
if __name__ == '__main__':
    ...
    print(solve_puzzle(test_inputs, rounds)) # 2713310158                                    

    actual_inputs = 'puzzle_inputs.txt' 
    print(solve_puzzle(actual_inputs, rounds)) # 24389045529